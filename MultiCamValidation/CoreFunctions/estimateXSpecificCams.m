...
%
% reconstructed = estimateXSpecificCams(loaded,IdMat,cam,shared_indexes)
%
...
% NEW FOR MOCAP:
% shared_indexes ... indexes of the cameras to use for the reconstruction
% no longer using tuple with the best reprojection error, but the common pair 
% of cameras between subgroups so alignment can be done
...
% NEW FOR MOCAP:
reconstructed.ptsIdx = find(sum(IdMat([shared_indexes(1), shared_indexes(2)],:))==2);
reconstructed.X		 = uP2X(Ws([tripleIdx{[shared_indexes(1), shared_indexes(2)]}],reconstructed.ptsIdx), [Pmat{[shared_indexes(1), shared_indexes(2)]}]);
reconstructed.CamIdx = [shared_indexes(1), shared_indexes(2)];
